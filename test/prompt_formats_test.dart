...
void main() {
  ...
  group('AgentPromptFormat', () {
    ...
    test('契约常量：输出标题与状态工具名', () {
      expect(AgentPromptFormat.outputSections, ['剧情演绎', '推荐行动']);
    ...
    test('systemHead 集中双语 7 条契约（工具名 / 锚定编辑 / 修复轮），末行为空行', () {
      final lines = format.systemHead;
      ...
      expect(zhCount, 7);
      expect(enCount, 7);
      // 关键规则与工具契约引用（中英双语规则 3、5 各出现一次）。
      expect(lines.any((l) => l.contains('narrchat_editSection')), isTrue);
      expect(lines.any((l) => l.contains('narrchat_advanceTime')), isTrue);
      ...
      expect(lines.any((l) => l.contains('修复轮纪律')), isTrue);
      expect(lines.last, '');
    });
    ...
    test('userExecuteNote 为双语【指令执行】', () {
      final lines = format.userExecuteNote;
      expect(lines, hasLength(2));
      expect(lines[0], contains('【指令执行】'));
      expect(lines[0], contains('全部四个状态工具'));
      expect(lines[1], contains('[Execute now]'));
      expect(lines[1], contains('ALL FOUR state tools'));
    });
  });
